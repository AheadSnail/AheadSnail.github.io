...
//utp 消息接受，填充buff  isNeedProcessData 如果为true，要判断是否接收的数据大于最大的buff限制，如果大于，多出来的数据，应该存储起来，等下次填充
void MSEHandshake::utpRecvDataProcess(const unsigned char *buff,size_t buffLen)
{
    //检查接受的内容的长度是否超过了最大的buff限制
    if (rbufLength_ >= MAX_BUFFER_LENGTH) {
        LOGD("MSEHandshake::utpRecvDataProcess bufLength_ >= MAX_BUFFER_LENGTH return");
        assert(!wantRead_);
        return;
    }
    ...
    //当前所能接受的大小
    size_t finallyLen = len > buffLen ? buffLen : len;
    A2_LOG_DEBUG(fmt("CUID#%" PRId64 " MSEHandshake::utpRecvDataProcess bufflen %d finallyLen %d maxLen %d",cuid_,buffLen,finallyLen,len));
    //读取内容,将读取的内容放到了rbuf_ 中 rbuf_ + rbufLength_代表指针的偏移
    memcpy(rbuf_+rbufLength_,buff,finallyLen);
    rbufLength_ += finallyLen;
    //wantRead_改为了false
    wantRead_ = false;

    //大于的化，将多余的部分存储到缓冲区中,也即是下次接收的值
    if(len < buffLen)
    {
        //将剩余内容，拷贝到缓冲空间
        memcpy(processMoreBuf_+processMoreBufLength_,buff+finallyLen,(buffLen - finallyLen));
        processMoreBufLength_ += buffLen - finallyLen;
        A2_LOG_DEBUG(fmt("CUID#%" PRId64 " processMoreBufLength_ %d",cuid_,processMoreBufLength_));
    }
}

上面会将当前传递的多余的buff存储到了另一个缓冲区中，当然，这个也不是多余的，是下一步要处理的内容
bool InitiatorMSEHandshakeCommand::executeInternal()
{
   //初始化的时候 wantRead_ 为false  ,后面等待数据的时候，会改为true
   if (mseHandshake_->getWantRead() && getSocket().get()->getSocketType() != SOCK_DGRAM) {
       //读取是否有数据到来
       mseHandshake_->read();
   }else{
       //utp 的情况 检查buff
       mseHandshake_->checkBuff();
   }
   ...
}

//下次检查缓冲空间的方法,将多余的内容拷贝到原本的buff中
void MSEHandshake::checkBuff()
{
    if(processMoreBufLength_ <= 0)
    {
        return;
    }
    //如果当前缓冲区中有内容,读取缓冲区的内容
    //检查接受的内容的长度是否超过了最大的buff限制,如果当前的rbufLength的内容还没有被消耗掉，应该返回，等待消耗
    if (rbufLength_ >= MAX_BUFFER_LENGTH) {
        LOGD("CUID#%" PRId64 " MSEHandshake::checkBuff bufLength_ >= MAX_BUFFER_LENGTH return",cuid_);
        assert(!wantRead_);
        return;
    }
    //读取内容,将读取的内容放到了rbuf_ 中 rbuf_ + rbufLength_代表指针的偏移
    memcpy(rbuf_+rbufLength_,processMoreBuf_,processMoreBufLength_);
    //移除当前当前已经添加到rbuf_的内容
    rbufLength_ += processMoreBufLength_;
    //重置为0
    processMoreBufLength_ = 0;
    //wantRead_改为了false
    wantRead_ = false;
}

PeerInteractionCommand 中也有类似的问题
//utp分发消息 统一接受
void PeerInteractionCommand::dispatchUtpMessage(const unsigned char *buff,size_t buffLen)
{
  //Bt 消息
  if(sequence_ == WIRED)
  {
      btInteractive_->utpRecvDataProcess(buff,buffLen, false);
  }else{
      //握手消息,注意这个回来的数据包由于utp的原因可能会将多个数据包一起合并返回来
      btInteractive_->utpRecvDataProcess(buff,buffLen, true);
  }
}

//utp 握手 接受内容处理,并且把数据返回到buff中，已经buffLen如果有加密的化，会改变  曹 因为utp的原因可能会将多条消息合并一起发过来
void PeerConnection::PeerUtpRecvDataProcess(const unsigned char *buff,size_t buffLen)
{
    //之前已经有过握手消息了
    if (BtHandshakeMessage::MESSAGE_LENGTH < resbufLength_) {
        LOGD("CUID#%" PRId64 " More than BtHandshakeMessage::MESSAGE_LENGTH bytes are buffered.",cuid_);
        throw DL_ABORT_EX("More than BtHandshakeMessage::MESSAGE_LENGTH bytes are buffered.");
    }
    //当前还可以写的最大长度
    ...
    if (remaining > 0) {
        ...
        if (encryptionEnabled_) {
            decryptor_->encrypt(buffLen, (unsigned char * )buff, buff);
        }

        //填充内容,将buff的内容填充到resbuf中,remaining为最多拷贝的长度
        memcpy(resbuf_.get() + resbufLength_,buff,remaining);

        if (remaining == 0 && !socket_->wantRead() && !socket_->wantWrite()) {
            // we got EOF  到了文件结尾，异常处理
            LOGD("CUID#%" PRId64" - In PeerConnection::receiveHandshake(), remain=%lu", cuid_, static_cast<unsigned long>(temp));
            peer_->setDisconnectedGracefully(true);
            throw DL_ABORT_EX(EX_EOF_FROM_PEER);
        }
        //当前已经写了多少个字节到 resbuf_ 中
        resbufLength_ += remaining;

        //代表含有其他的消息一起返回,要相应的解析,将多余的内容拷贝到其他的缓冲区中
        if(buffLen > 68)
        {
            //填充内容,将buff的内容填充到resbuf中
            memcpy(btResbuf_.get()+btResBufLength_,buff+resbufLength_,buffLen-resbufLength_);
            btResBufLength_ += (buffLen -resbufLength_);
        }

    }
}

//utp bt消息 如果当前的消息有加密，要解密处理，
void PeerConnection::PeerUtpBtRecvDataProcess(const unsigned char *buff,size_t buffLen)
{
    //判断是否有加密
    if (encryptionEnabled_) {
        decryptor_->encrypt(buffLen, (unsigned char * )buff, buff);
    }
    //填充内容,将buff的内容填充到resbuf中,btResBufLength_ 如果不为0，那就是前面传递的内容中还有包含下一部分的内容, 不能覆盖掉，要在原有的基础上添加
    memcpy(btResbuf_.get()+btResBufLength_,buff,buffLen);
    btResBufLength_ += buffLen;
    //LOGD("PeerConnection::PeerUtpBtRecvDataProcess buffLen %d btResBufLength_ %d",buffLen,btResBufLength_);
}

//还有一个就是 PeerConnection::receiveMessage 中接受消息，要参考他原有的逻辑
...
{
  //BT标准BEP-3中定义了8种peer消息：消息格式为msg_len(4个字节) + msg_type(1一个字节，也叫id) + payload(用来填充对应的消息对应的内容，也即是不共用的部分)
  while (1) {
    bool done = false;
    size_t i;
    //resbufOffset_ 代表 resbuf_ 中已经处理了多少个字节 默认为0
    //resbufLength_ 标识当前已经写了多少个字节到resbuf_中
    for (i = resbufOffset_; i < resbufLength_ && !done; ++i) {
      unsigned char c = resbuf_[i];
      switch (msgState_) {//msgState_ 默认为 BT_MSG_PREV_READ_LENGTH ，在初始化这个对象的时候赋值为这个值
      case (BT_MSG_PREV_READ_LENGTH):
        msgOffset_ = i;//标识当前消息的起始地址偏移量，
        currentPayloadLength_ = 0;
        msgState_ = BT_MSG_READ_LENGTH;
      // Fall through
      case (BT_MSG_READ_LENGTH):
        //读取length,由于currentPayloadLength为32个字节的变量，所以每读取一个字节就要对应的左移一位,由于报文length都是占四个字节，所以刚刚好
        currentPayloadLength_ <<= 8;
        currentPayloadLength_ += c;

        // The message length is uint32_t    也即是到了msg_len的内容(4个字节)
        if (i - msgOffset_ == 3) {
          //判断是否超过了最大的缓冲空间的大小
          if (currentPayloadLength_ + 4 > bufferCapacity_) {
              LOGD(EX_TOO_LONG_PAYLOAD, currentPayloadLength_);
              throw DL_ABORT_EX(fmt(EX_TOO_LONG_PAYLOAD, currentPayloadLength_));
          }

          //LOGD("current message length %d",currentPayloadLength_);

          //length为0代表为 keep-alive 的消息
          if (currentPayloadLength_ == 0) {
            // Length == 0 means keep-alive message.
            //当前读取的消息为keep-alive消息，这里标识消息读取完毕,标识done为true,可以解析当前的这条消息了,这里的for循环就会退出了
            done = true;
            msgState_ = BT_MSG_PREV_READ_LENGTH;
          }
          else {
            //标识下一次读取的状态为BT_MSG_READ_PAYLOAD ，也即是读取payload的内容 也即是消息特有的部分
            msgState_ = BT_MSG_READ_PAYLOAD;
          }
        }
        break;
      case (BT_MSG_READ_PAYLOAD)://读取payload
        ...
        if (resbufLength_ - msgOffset_ >= 4 + currentPayloadLength_) {//如果当前接受的内容长度大于当前消息的长度
          //标识让i为下一个读取的位置，也即是在当前消息之后的一个字节开始读取,因为完全有可能下一次消息在当前消息的后面
          i = msgOffset_ + 4 + currentPayloadLength_ - 1;
          //LOGD("enought data next message i %d resbufLength_ %d",i,resbufLength_);
          //标识完成,这里的for循环就会退出了,在这里也会出现一种当满足了这个条件，而且下一次读取的内容i到了buff的末尾
          done = true;
          //重置变量，后面再次的解析剩下的内容,也即是下次再来重新的解析后面的消息内容
          msgState_ = BT_MSG_PREV_READ_LENGTH;
        }
        else {
          // We need another read.  如果不是，说明内容不够，我们需要再次的读取内容,让i为当前读取到最后一个字节，所以下次有内容的时候，可以接着读取内容
          //注意此时的状态还是为 BT_MSG_READ_PAYLOAD ,所以下次会直接判断是否读取到了足够的长度,并不会重新的开始
          i = resbufLength_ - 1;
        }
        break;
      }
    ...
    //下面的代码为for循环之外,赋值resbufOffset_ = i,标识下一次应该要读取的开始位置
    resbufOffset_ = i;

    //标识当前已经解析到了一条消息,外界可以处理这条消息了
    if (done) {
      //如果完成，返回，处理当前解析的内容,也即是当前解析到了一条消息了,可以处理了
      if (data) {
        memcpy(data, resbuf_.get() + msgOffset_ + 4, currentPayloadLength_);
      }
      //赋值dataLength = currentPayloadLength_ 为当前消息的长度 return true 代表当前消息解析完毕,通知外面可以解析这条消息了
      dataLength = currentPayloadLength_;
      //LOGD("done true dataLength %d resbufOffset_  %d",currentPayloadLength_,resbufOffset_);
      return true;
    }
    else {
      //没有完成，可能是当前接收的数据长度不够,或者 当前刚好数据解析完毕，也会到这里
      //确保resbufOffset_ 跟 resbufLength_一样，也即是将读取的内容都处理了
      assert(resbufOffset_ == resbufLength_);
      if (resbufLength_ != 0) {

        //出现这种问题是因为当发送的内容，刚好是完整的一条消息的时候，下一次触发的i为buff的末尾，此时done为false，但是他此时是一个完整的消息了
        //所以这里就是用来处理这种情况，所以这里就可以用来重置变量了
        if (resbufLength_ - msgOffset_ == currentPayloadLength_ + 4) {//如果所有的数据都已经解析完成，重置变量
          // All bytes in buffer have been processed, so clear it
          // away.
          //LOGD("data just enought reset Data resbufLength_ %d msgOffset_ %d currentPayloadLength_ %d",resbufLength_,msgOffset_,currentPayloadLength_);
          resbufLength_ = 0;
          resbufOffset_ = 0;
          msgOffset_ = 0;
        }
          ...
          //这里的意思就是将当前消息起始位置的数据拷贝到   resbuf_中的 首部，拷贝的字节为resbufLength_ - msgOffset_,这样拷贝之后,就可以将 resbufLength_ -= msgOffset_;
          //以及标识   resbufOffset_ = resbufLength_; 当下次再读取数据的时候， resbuf_.get() + resbufLength_ 就不会覆盖掉我们当前消息剩余的内容，牛逼!!!
          memmove(resbuf_.get(), resbuf_.get() + msgOffset_, resbufLength_ - msgOffset_);
          //让缓冲区当前的长度减去已经读取的消息的长度
          resbufLength_ -= msgOffset_;
          //标识下一次消息读取的起始位置
          resbufOffset_ = resbufLength_;
          msgOffset_ = 0;
          //LOGD("not enought data left length %d next message startOffese %d",resbufLength_,resbufOffset_);
        }
      }

      //由于数据长度不够，所以这里要再次的读取内容
      //nread代表当前最大的能接受的数据的长度，最大不能超过 bufferCapacity_  缓冲空间的大小
      size_t nread;
      // To reduce the amount of copy involved in buffer shift, large
      // payload will be read exactly.  减少缓冲移位所涉及的复制量
      ...
      //utp 直接从缓冲区中获取数据
      if(getSocketBuffer()->getSocketCore()->getSocketType() == SOCK_DGRAM)
      {
          //缓冲区中有内容才要拷贝
          if(btResBufLength_ != 0)
          {
              ...
              //LOGD("utp receiveMessage before btResBufLength_ %d after leftLength %d resbufLength_ %d",btResBufLength_,leftLength,resbufLength_);
              btResBufLength_ -= finallyLen;
          } else{
              //缓冲区没有内容，中断循环，下次继续执行
              //LOGD("btResBufLength_ == 0 break");
              break;
          }
      }else{

          //读取Bt消息，当前消息的读取起始位置为 resbuf_.get() + resbufLength_ , 因为 当前消息如果内容不够的化，就会将剩下的长度移植到这个缓冲区的前面，同时resbufLength_ 也记录了当前消息剩下的
          //内容，所以下次读取的时候，应该在这个后面添加内容，不能覆盖掉
          ...
          if (nread == 0) {
              ...
              //标识当前缓冲区已经读取的长度
              resbufLength_ += nread;
              LOGD("PeerConnection  readData %d btResBufLength_ %d",nread,resbufLength_);
          }
      }
    }
  }
